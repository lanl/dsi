...
class DUCK_DB:
    """
    DuckDB Filesystem Backend to which a user can ingest/process data, generate a Jupyter notebook, and find all occurences of a search term
    """
    runTable = False
    ...
    def ingest_table_helper(self, types, foreign_query = None, isVerbose=False):
        ...
        **Users do not interact with this function and should ignore it. Called within ingest_artifacts()**
        ...
        `return`: none
        """
        if self.cur.execute(f"SELECT table_name FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_name = '{types.name}'").fetchone():
    ...
    def ingest_artifacts(self, collection, isVerbose=False):    
        ...
        if self.runTable:
            runTable_create = "CREATE TABLE IF NOT EXISTS runTable (run_id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY, run_timestamp TEXT UNIQUE);"
    ...
    def process_artifacts(self, only_units_relations = False, isVerbose = False):
        """
        Reads in data from the DuckDB database into a nested Ordered Dictionary, where keys are table names and values are Ordered Dictionary of table data.
        If there are PK/FK relations in a database it is stored in a table called `dsi_relations`.

        Can only be called if a loaded DuckDB database is a BACK-READ backend in a Core.Terminal workflow (check core.py for distinction)
        ...
        tableList = self.cur.execute('SELECT table_name FROM information_schema.tables WHERE table_schema = \'main\' AND table_type = \'BASE TABLE\'').fetchall()
        pkList = []
