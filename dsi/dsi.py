...
class DSI:
    ...
    def __init__(self, filename = ".temp_dsi.db", backend_name = "Sqlite", **kwargs):
        ...
        `filename` : str, optional, default is ".temp_dsi.db"
            ...
            Accepted file extensions for DSI-compatible backends:
                ...
                - If backend_name = "Maglab" → No filename input (read-only backend)

        `backend_name` : str, optional, default is "Sqlite".
            Name of the backend to activate.

            If using a DSI-supported backend, must be either "Sqlite", "DuckDB", "NDP", "OSTI", "Oceans11", "RCSBPDB", or "Maglab".
        ...
        if backend_name.endswith(".py"):
            ...
            if backend_module is None:
                raise RuntimeError("Please check the 'backend_name' argument as it is not supported by DSI\n"
                                    "Eligible backend_names are: Sqlite, DuckDB, NDP, OSTI, Oceans11, RCSBPDB, Maglab")
            ...
            if self.read_only_flag:
                ...
                if backend_name.lower() == "ndp":
                    ...
                    query_params = kwargs.pop("params", {})
                elif backend_name.lower() == "maglab":
                    backend_name = "Maglab"
                    query_params = kwargs.pop("params", {})
                else:
                    raise NotImplementedError("The currently supported read-only backends are NDP, OSTI, RCSBPDB, Oceans11 and Maglab")
    ...
    def list_backends(self):
        ...
        n = Maglab(only_validate=True)
        if n.validate_connection():
            print("Maglab : Read-only metadata backend for discovering and querying MagLab datasets published on OSF.")
        print()
