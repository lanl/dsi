...
def test_bueno_plugin_type():
    ...
    plug = Bueno(filenames=path, perms_manager=mock_pm)
    plug.add_rows()
...
def test_bueno_plugin_adds_rows():
    ...
    plug = Bueno(filenames=[path1, path2], perms_manager=mock_pm)
    plug.add_rows()
    plug.add_rows()
...
def test_csv_plugin_type():
    ...
    plug = Csv(filenames=path, perms_manager=mock_pm)
    plug.add_rows()
    assert type(plug.output_collector) == OrderedDict
...
def test_csv_plugin_adds_rows_multiple_files():
    ...
    plug = Csv(filenames=[path1, path2], perms_manager=mock_pm)
    plug.add_rows()
...
def test_csv_plugin_adds_rows_multiple_files_strict_mode():
    ...
    plug = Csv(filenames=[path1, path2], strict_mode=True, perms_manager=mock_pm)
    try:
        plug.add_rows()
    except TypeError:
